// verifier-joueurs.mjs  (v3 — clubs uniquement via filtre RAPIDE, sélections nationales écartées côté script)
// ─────────────────────────────────────────────────────────────────────────────
...
// Requête : les passages en équipe (P54) ; les sélections nationales sont écartées ensuite par estSelection().
...
// Vraie sélection nationale (à écarter) ? Repérée par son nom (labels fr/en).
function estSelection(nom){
  return /^équipe d|^équipe nationale|sélection nationale|national football team|national association football team|women'?s national|olympic football team|équipe.*olympique/i.test(nom||"");
}
...
function deduire(rows, qid){
  ...
  for(const b of rows){
    ...
    if(b.teamLabel && !estSelection(b.teamLabel.value)){
      spells.push({
